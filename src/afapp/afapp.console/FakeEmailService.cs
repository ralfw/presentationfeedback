...
namespace afapp.console
{
	internal class FakeEmailService : IEmailService
	{
		public void Send_speaker_notification(SpeakerNotificationData data)
		{
			...
			Console.WriteLine("Thank you for speaking at the conference {0}", data.ConferenceTitle);
			Console.WriteLine("Your session {0} on the {1} received the follows scores:", 
				data.Session.Title, data.Session.Start.ToShortDateString() );
			Console.WriteLine("Red: {0}", data.Reds);
			Console.WriteLine("Yellow: {0}", data.Yellows);
			Console.WriteLine("Green: {0}", data.Greens);
